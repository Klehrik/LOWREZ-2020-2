...
else
{
	if (SetStats == 0)
	{
		...
		var image_table = [0, 1, 0, 1, 0, 1, 2, 0, 0, 0, 0, 0, 0, 0, 0];
		...
		var max_travel_table = [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 14, 16];
		...
		if (sprite_index < 0)
		{
			if (LaserEndX == "")
			{
				...
				while (point_distance(x, y, LaserEndX, LaserEndY) < 128)
				{
					...
					if (position_meeting(LaserEndX, LaserEndY, obj_Asteroid))
					{
						...
						if (obj > -1)
						{
							...
							if (Collide == 1) break;
							else if (Collide == 2) { break; instance_destroy(obj); }
							else if (Collide == 3) instance_destroy(obj);
						}
					}
					
					if (position_meeting(LaserEndX, LaserEndY, obj_PlayerProj))
					{
						var obj = instance_position(LaserEndX, LaserEndY, obj_PlayerProj);
						
						if (obj > -1)
						{
							instance_create_depth(LaserEndX, LaserEndY, 0, obj_Hit);
						
							if (obj.Collide == 3) break;
							else if (obj.Collide == 2) { break; instance_destroy(obj); }
							else if (obj.Collide == 1) instance_destroy(obj);
						}
					}
					
					if (position_meeting(LaserEndX, LaserEndY, obj_Player))
					{
						if (global.PlayerShield > 0) { global.PlayerShield = clamp(global.PlayerShield - ShieldDamage, 0, global.PlayerMaxShield); if (obj_Player.ShieldFull == 0) obj_Player.ShieldRegen = 180; }
						else global.PlayerHP -= Damage;
